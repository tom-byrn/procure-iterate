...
const ChatInput = ({
  ...
  return <form onSubmit={handleSubmit} className="relative">
      <div className="flex items-end gap-2 rounded-3xl border border-input bg-card p-3 shadow-medium transition-all duration-300 focus-within:border-primary focus-within:shadow-lg">
        ...
        <Button type="submit" size="icon" disabled={!message.trim() || disabled} className={cn("shrink-0 rounded-xl bg-gradient-primary transition-all duration-300 hover:brightness-90", message.trim() && !disabled ? "scale-100 opacity-100" : "scale-90 opacity-50")}>
